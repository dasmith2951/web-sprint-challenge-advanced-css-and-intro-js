...
function getArtistByIndex(artistArray, artistIndex) {
  let artistName = artistArray[artistIndex].name;
  return `the artist at index ${artistIndex} is ${artistName}`;
}  
...
function get20s(get20sArray){
  const bornDied20thCenturyArtist = [];
  for (let i = 0; i < get20sArray.length; i++) {
    if (get20sArray [i] ['years'].split (" - ") [0] >= "1900" &&
      get20sArray [i] ['years'].split (" - ") [1] <= "2000") {
      bornDied20thCenturyArtist.push(get20sArray[i]['name']);
    }
  }
  return bornDied20thCenturyArtist;
}
...
function removeArtist(removeArtistArray, removeArtistIndex){
  removeArtistArray.splice(removeArtistIndex,1)
  
  return removeArtistArray.length;
}
...
function addArtist(newArray, newId, newName, newYears, newGenre, newNationality, newBio){
  newArray.push({id: newId, name: newName,years: newYears, genre: newGenre, nationality: newNationality, bio: newBio});
  return newArray;
}

console.log(addArtist(artists, 20, "David Smith", "1990 - 4/4/2021", "Web Design", "American", "Originally from Nashville,TN but moved to Durham, NC to pursue Web Design."));  
...
function lotsOfArt(lotsOfArtArray){
  const hundredsPaintingsArray = [];
  for (let i = 0; i < lotsOfArtArray.length; i++) {
    if (lotsOfArtArray[i].paintings >= 100) {
      hundredsPaintingsArray.push(lotsOfArtArray[i].name);
    }
  }
  return hundredsPaintingsArray;
}
